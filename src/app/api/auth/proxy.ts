...
const BACKEND = (
  process.env.BEO_API_BASE || 'http://127.0.0.1:47500'
).replace(/\/$/, '');
...
function appendUpstreamCookies(
...
export async function proxyAuthPost(
  ...
  if (options.isSignup) {
    headers['x-signup-key'] =
      process.env.SIGNUP_PROXY_SECRET || '';

    headers.origin =
      process.env.SIGNUP_ALLOWED_ORIGIN ||
      req.headers.get('origin') ||
      '';
  }

  const upstream = await fetch(
    `${BACKEND}/v1/auth/${authPath}`,
    {
      method: 'POST',
      headers,
      body,
